//
//  geometry_textured_boned_mesh.h
//
...
namespace renderer {
  ...
  struct TexturedBonedMeshVertex {
    GLfloat pos[3];
    GLfloat norm[3];
    GLfloat tex[2];
  ...
  struct TexturedBonedMeshFileData {
    ...
    // Vertex, normal, UV, bone_data, index data to follow
  };

  class GeometryTexturedBonedMesh : public Geometry {
  public:
    friend class GeometryManager;

    // Constructor / Destructor
    GeometryTexturedBonedMesh();
    virtual ~GeometryTexturedBonedMesh();
    ...
    // Virtual methods
    virtual void draw();
    inline virtual GeometryType type() { return GEOMETRY_TEXTURED_BONED_MESH; }
  ...
  private:
    ...
    static GeometryTexturedBonedMesh* convertAssimpMesh(const std::string& path, 
    ...
    GeometryTexturedBonedMesh& operator=(const GeometryTexturedBonedMesh&);
  };
